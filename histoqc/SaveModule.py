...
import numpy as np
from skimage import measure, io, color, img_as_ubyte  
    ...
    for name in os.listdir(od):
        ...
        if os.path.isfile(file):
            if name.endswith('.tsv'):
                ...
                mask.setWidth(rdouble((oim.getSizeX()-mask.getX()._val)/4)); mask.setHeight(rdouble((oim.getSizeY()-mask.getY()._val)/4))
...
def scale(A, B):     # fill A with B scaled by k
    Y = A.shape[0]
    X = A.shape[1]
    ky = Y//B.shape[0]
...
def __create_polygon(contour, x_offset=0, y_offset=0, z=None, t=None):
    ...
    for name in os.listdir(od):
        ...
        if os.path.isfile(file):
            if name.endswith('.tsv'):
                ...
                np_arr=io.imread(file).astype(bool)
                np_arr2=np.zeros((oim.getSizeY(),oim.getSizeX()), dtype=bool)
                scale(np_arr2,np_arr)
                contours = measure.find_contours(np_arr2)
                if len(contours) > 0:
                    contour = get_longest_contour(contours)
                    # Only add 1 Polygon per Mask Shape.
                    # First is usually the longest
                    polygon= __create_polygon(contour)
